...
public class CandidateTest {
    ...
    @Mock
    private RaftStateContext mockRaftStateContext;
    ...
    public void testRequestVoteWithNewerTerm() throws Exception {
        ...
        RequestVote request = RequestVote.newBuilder().setCandidateId(mockCandidate.toString()).setLastLogIndex(1L).setLastLogTerm(1L).setTerm(4L).build();
    ...
    public void testAppendEntriesWithNewerTerm() throws Exception {
        ...
        AppendEntries request = AppendEntries.newBuilder().setTerm(4L).setLeaderId(mockLeader.toString()).setPrevLogIndex(1L).setPrevLogTerm(1L).setCommitIndex(1L).build();
    ...
    public void testAppendEntriesWithSameTerm() throws Exception {
        Candidate candidate = new Candidate(mockRaftLog, mockScheduler, 1, mockRaftClient);
        candidate.init(mockRaftStateContext);
        AppendEntries request = AppendEntries.newBuilder().setTerm(2L).setLeaderId("leader").setPrevLogIndex(1L).setPrevLogTerm(1L).setCommitIndex(1L).build();
        candidate.appendEntries(mockRaftStateContext, request);
        verify(mockRaftLog).currentTerm(3L);
        verify(mockRaftLog, times(1)).currentTerm(anyLong());
        verify(mockRaftLog).lastVotedFor(Optional.of(self));
